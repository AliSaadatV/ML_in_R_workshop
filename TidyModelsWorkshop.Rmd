...
## Extra: Running a single recipe against a single model ()

```{r}
# We will now create the workflow, which combines the preprocessed data (recipe) with our defined ML model (engine).

lr_workflow <- 
  workflow() %>% 
  add_model(xgboost_spec) %>% 
  add_recipe(data_rec_mean)
